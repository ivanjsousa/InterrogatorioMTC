...
def main(page: ft.Page):
    ...
    def save_data(_):
        ...
        page.snack_bar = ft.SnackBar(ft.Text(f"Dados salvos com sucesso em {filename}!"))
        page.snack_bar.open = True
        page.update()

    # Função para carregar e exibir interrogatório
    def print_interrogatorio(e: ft.FilePickerResultEvent):
        if not e.files:
            return
        ...
        try:
            ...
            # Criar conteúdo formatado para exibição
            content = []
            for section, items in data.items():
                content.append(ft.Text(section.upper(), weight=ft.FontWeight.BOLD, size=16))
                
                if isinstance(items, dict):
                    for key, value in items.items():
                        if value:  # Só mostra se tiver valor
                            content.append(ft.Text(f"{key.replace('_', ' ').title()}: {value}"))
                else:
                    content.append(ft.Text(str(items)))
                
                content.append(ft.Divider())
            
            # Criar diálogo para exibir o conteúdo
            dialog = ft.AlertDialog(
                title=ft.Text("Interrogatório Salvo"),
                content=ft.Column(content, scroll=ft.ScrollMode.ADAPTIVE, height=500, width=600),
                actions=[
                    ft.TextButton("Fechar", on_click=lambda _: page.close_dialog()),
                    ft.TextButton("Salvar PDF", on_click=lambda _: save_as_pdf(content))
                ],
            ...
            page.snack_bar = ft.SnackBar(ft.Text(f"Erro ao carregar arquivo: {str(ex)}"))
            page.snack_bar.open = True
            page.update()

    def save_as_pdf(_):
        # Implementar a lógica para salvar como PDF aqui
        page.snack_bar = ft.SnackBar(ft.Text("Funcionalidade de PDF ainda não implementada"))
        page.snack_bar.open = True
        page.update()

    # File picker para selecionar arquivo JSON
    file_picker = ft.FilePicker(on_result=print_interrogatorio)
    page.overlay.append(file_picker)

    # Botão para imprimir interrogatório
    btn_print = ft.ElevatedButton(
        "Visualizar Interrogatório Salvo",
        icon=ft.Icons.PRINT,
        on_click=lambda _: file_picker.pick_files(
            allowed_extensions=["json"],
            dialog_title="Selecione um interrogatório salvo"
        ),
    ...
    def toggle_saude_reprodutiva(_):
    ...
    def update_saude_reprodutiva(_):
    ...
    def update_emocoes(_):
        emocoes_selecionadas = [item.label for item in checkbox_emocoes if item.value]
        interrogatorio["emocoes"] = {
            "emoções_predominantes": ", ".join(emocoes_selecionadas),
            "situacoes_estresse": situacoes_estresse.value,
            "ansiedade": ansiedade.value,
            "estado_emocional": estado_emocional.value
        }
    ...
    cor_lingua = ft.Dropdown(
        ...
        options=[
            ...
            ft.dropdown.Option("vermelha-encarnado"),
    ...
    saburra = ft.Dropdown(
        ...
        options=[
            ...
            ft.dropdown.Option("fina ou ausente"),
            ft.dropdown.Option("cinza ou preta"),
            ft.dropdown.Option("pegajosa")
        ]
    )
    ...
    # Lista com os checkboxes individuais
    checkboxes_forma_lingua = [
        ft.Checkbox("rígida"),
        ft.Checkbox("fina"),
        ft.Checkbox("inchada"),
        ft.Checkbox("fissurada"),
        ft.Checkbox("trêmula"),
        ft.Checkbox("com marcas de dentes")
    ]

    # Organização em 2 colunas
    forma_lingua = ft.Row(
        controls=[
            ft.Column(controls=checkboxes_forma_lingua[:3], spacing=5),
            ft.Column(controls=checkboxes_forma_lingua[3:], spacing=5)
        ],
        spacing=20
    )
    ...
    # Função para atualizar os dados da seção
    def update_lingua_pulso(_):
        interrogatorio["lingua_pulso"] = {
            "cor_lingua": cor_lingua.value,
            "saburra": saburra.value,
            "umidade": umidade_lingua.value,
            "forma": ", ".join(item.label for item in checkboxes_forma_lingua if item.value),
            "pulso": pulso.value
        }

    # Associa os eventos aos campos
    for field in [cor_lingua, saburra, umidade_lingua, pulso]:
        if hasattr(field, 'on_change'):
            field.on_change = update_lingua_pulso
        else:
            field.on_blur = update_lingua_pulso

    # Os checkboxes precisam de eventos individuais
    for cb in checkboxes_forma_lingua:
        cb.on_change = update_lingua_pulso
    ...
    def update_estacoes_horarios(_):
    ...
    page.add(
        ft.Column([
            ...
            ft.Divider(),
            ft.Row([btn_salvar, btn_print], spacing=20)
        ], spacing=20)
    )
